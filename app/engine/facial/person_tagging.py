from app.engine.facial.utils import (
    list_files,
    list_directories,
    write_dict_to_json,
    read_json_to_dict,
...
def find_similar_person(folderpath):
    ...
    for picture in pictures:
        picture_faces = list_files(
            os.path.join(current_app.config["TEMP_FOLDER_PATH"], picture)
        )
        report[picture] = {}
        for picture_face in picture_faces:
            ...
            for other_picture in other_pictures:
                ...
                other_picture_faces = list_files(
                    os.path.join(current_app.config["TEMP_FOLDER_PATH"], other_picture)
                )
                for other_picture_face in other_picture_faces:
                    ...
                    right_face = os.path.join(
                        current_app.config["TEMP_FOLDER_PATH"],
                        other_picture,
                        other_picture_face,
                    )
                    if (
                        left_face + right_face not in cache
                        and right_face + left_face not in cache
                    ):
                        report[picture][picture_face][other_picture][
                            other_picture_face
                        ] = float(calculate_face_similarity(left_face, right_face))
...
def tag_people_in(directory, image_names, user_id, threshhold=0.5):
    ...
    known_faces_dir_path = os.path.join(os.getcwd(), "content", user_id, "_known_faces")
    ...
    for known_face in known_faces:
        ...
        for other_image in other_images:
            other_faces = list_files(
                os.path.join(current_app.config["TEMP_FOLDER_PATH"], other_image)
            )
            for other_face in other_faces:
                ...
                if calculate_face_similarity(image_a, image_b) > threshhold:
                    ...
                    person_information = {
                        "name": known_data[person_id]["name"],
                        "personId": person_id,
                    }
